...
class ApiV1AdmMemberControllerTest @Autowired constructor(
    ...
    fun t1() {
        val resultActions = mvc.perform(get("/api/v1/adm/members")).andDo(print())
        ...
        for ((i, member) in members.withIndex()) {
            resultActions
                ...
                .andExpect(jsonPath("$[$i].createDate").value(Matchers.startsWith(member.createDate.toString().substring(0, 20))))
                .andExpect(jsonPath("$[$i].modifyDate").value(Matchers.startsWith(member.modifyDate.toString().substring(0, 20))))
    ...
    fun t2() {
        val id = 1
        val resultActions = mvc.perform(get("/api/v1/adm/members/$id")).andDo(print())
